...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False):
        ...
        # Pointwise Convolution: Use 1x1 convolution to combine features from various channels
...
class BasicBlockGhost(nn.Module):
    ...
    def forward(self, x):
        residual = x  # Retain the input for residual connections
        ...
        if self.downsample is not None:
            residual = self.downsample(x)  # If downsampling is required, downsample the residuals
...
class ResNet18(nn.Module):
    def __init__(self, block, layers, num_classes=90, type=1, scale=4, scale_flag=False):
        ...
        self.scale = scale
        self.in_channels = 64
        self.conv1 = nn.Conv2d(1, 64, kernel_size=(2, 7), stride=2, padding=(0, 3), bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)

...
class Res2NetGCModule(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, scale=4, stype='normal'):
        ...
        self.conv3 = nn.Conv2d(width * scale, planes, kernel_size=1, stride=stride, bias=False)
...
class Res2NetModule(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, scale=4, stype='normal'):
        ...
        if scale == 1:
            ...
            self.nums = scale - 1
        if stype == 'stage':
            self.pool = nn.AvgPool2d(kernel_size=(1, 3), stride=stride, padding=(0, 1))
        convs = []
        bns = []
        for i in range(self.nums):
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False):
        ...
        # Pointwise Convolution: Use 1x1 Convolution combines the features of each channel
...
class BasicBlockGhost(nn.Module):
    ...
    def forward(self, x):
        residual = x  # Retain the input for residual connection
        ...
        if self.downsample is not None:
            residual = self.downsample(x)  # If downsampling is required, downsample the residual.
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        """
        Basic Module, There are two forms in total, 1.s=1 When the input and output dimensions are the same 2.s=2 The size of the feature map is reduced by half, the dimension is doubled
...
class ResNet10(nn.Module):
    def __init__(self, block, layers, num_classes=90):
        ...
        self.in_channels = 64
        self.conv1 = nn.Conv2d(1, 64, kernel_size=(2, 7), stride=1, padding=(0, 3), bias=False)
        ...
        if params.metric == 'add_margin':
            self.metric_fc = AddMarginProduct(256 * block.expansion, num_classes, s=16, m=0.2)
        elif params.metric == 'arc_margin':
            self.metric_fc = ArcMarginProduct(256 * block.expansion, num_classes, s=30, m=0.5,
                                              easy_margin=params.easy_margin)
        elif params.metric == 'sphere':
            self.metric_fc = SphereProduct(256 * block.expansion, num_classes, m=4)
        else:
            self.metric_fc = nn.Linear(256 * block.expansion, num_classes)

        # Initialization parameters -> Affect accuracy 7%
...
class ResNet18(nn.Module):
    def __init__(self, block, layers, num_classes=90, type=1):
        super(ResNet18, self).__init__()
        self.type = type  # 1 represents classics，0 represents res2net
        self.in_channels = 64
    ...
    def _make_layer(self, block, out_channels, blocks, stride=1):
        ...
        #     layers.append(block(self.in_channels, out_channels, stride, downsample))
        # else:
        #     layers.append(block(self.in_channels, out_channels, stride, downsample, stype='stage'))
        layers.append(block(self.in_channels, out_channels, stride, downsample))
...
def res2netgc_resnet18(num_classes=16):
    return ResNet18(Res2NetGCModule, [2, 2, 2, 2], num_classes=num_classes, type=0)


def res2net_resnet18(num_classes=16):
    return ResNet18(Res2NetModule, [2, 2, 2, 2], num_classes=num_classes, type=0)
...
if __name__ == '__main__':
    ...
    # model3 = res2net_resnet18(num_classes=16).cuda()
    # model4 = res2netgc_resnet18(num_classes=16).cuda()
    # model5 = resnet10(num_classes=16).cuda()
    # model6 = res2netgc_resnet18(num_classes=16).cuda()
    ...
    # summary(model5, (16, 2, 4800))
    # summary(model6, (16, 2, 4800))
    # print(outputs.shape)
